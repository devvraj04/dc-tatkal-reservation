...
public class BookingServiceImpl extends UnicastRemoteObject implements BookingService {
    ...
    public BookingServiceImpl() throws RemoteException {
        super();
        System.out.println("BookingServiceImpl: Remote object instantiated.");
    }
    ...
    // 4. Book Tatkal ticket (locks available seats using transaction and FOR UPDATE to prevent race conditions)
    @Override
    public BookingResult bookTatkalTicket(long userId, long scheduleId, String coachType, List<PassengerInput> passengers, String paymentMode) throws RemoteException {
        System.out.println(String.format("Server: Booking request. User: %d | Schedule: %d | Coach: %s | Passengers: %d", 
            userId, scheduleId, coachType, passengers.size()));
        ...
        try {
            ...
            // 2. Query available seats and LOCK them for update to prevent double-booking
            String seatQuery = "SELECT sa.allocation_id, sa.seat_id, s.seat_number, c.coach_number, s.berth_type " +
                               ...
                               "LIMIT ? " +
                               "FOR UPDATE"; // Exclusive lock on selected seat rows
            ...
            for (PassengerInput pi : passengers) {
                ...
                if (passId == -1) {
                    ...
                    // Fetch passenger name for history DTO representation
            ...
            // If we have enough seats for all, booking status = CONFIRMED, else waitlisted (or partially)
            ...
            // 7. Update Seat Allocations and Insert Booking Passengers
            int allocatedCount = lockedSeats.size();
            for (int i = 0; i < passengers.size(); i++) {
                ...
                if (i < allocatedCount) {
                    ...
                    // Insert Booking Passenger (no seat details)
                    String insertBp = "INSERT INTO booking_passengers (pnr, passenger_id, allocation_id, passenger_status, fare) " +
                                      "VALUES (?, ?, NULL, 'WAITING', ?)";
                    try (PreparedStatement ps = conn.prepareStatement(insertBp)) {
                        ps.setString(1, pnr);
                    ...
                    // Determine waitlist number (GNWL or TQWL)
                    int nextWlNo = 1;
            ...
            System.out.println("Server: Booking transaction committed for PNR: " + pnr);
            ...
            if (conn != null) {
                try {
                    conn.rollback();
                    System.err.println("Server: Transaction rolled back due to error.");
                } catch (SQLException ex) {
                    ex.printStackTrace();
                }
            }
            e.printStackTrace();
            throw new RemoteException("Transaction failed: " + e.getMessage());
        } finally {
            if (conn != null) {
                try {
